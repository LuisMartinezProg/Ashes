...
_openSub(type) {
    ...
    Object.assign(backBtn.style, {
      ...
      color: CHARACTER_MENU.gold, fontSize: '20px',
  ...
  _fillAtributos(body) {
    ...
    for (const { icon, label, value } of defs) {
      ...
      row.innerHTML = `
        ...
        <span style="font-family:monospace;font-size:12px;color:${CHARACTER_MENU.gold};font-weight:bold">${value}</span>
      `;
    ...
    Object.assign(xpFill.style, {
      height    : '100%',
      width     : atMax ? '100%' : `${pct}%`,
      background: atMax ? CHARACTER_MENU.xpMaxGrad : CHARACTER_MENU.xpGradient,
    ...
    Object.assign(plusBtn.style, {
      ...
      color         : atMax ? 'rgba(201,168,76,0.2)' : CHARACTER_MENU.gold,
      fontSize      : '14px',
      lineHeight    : '1',
  ...
  _openLevelUpPanel(parentBody) {
    ...
    matLeft.innerHTML = `
      ...
      <span style="font-family:monospace;font-size:10px;color:${CHARACTER_MENU.gold}">Núcleo Arcano</span>
    `;
    ...
    Object.assign(matRight.style, {
      fontFamily: 'monospace',
      fontSize  : '11px',
      color     : canConfirm ? CHARACTER_MENU.gold : CHARACTER_MENU.danger,
    });
  ...
  _fillEquipo(body, panel) {
    ...
    for (const slot of slots) {
      ...
      info.style.flex = '1';
      info.innerHTML  = `
        <div style="font-family:monospace;font-size:11px;color:${item ? CHARACTER_MENU.gold : 'rgba(201,168,76,0.3)'}">
      ...
      Object.assign(btn.style, {
        ...
        color       : CHARACTER_MENU.gold, fontFamily: 'monospace',
    ...
    for (const [stat, val] of Object.entries(total)) {
      ...
      cell.style.cssText = `background:rgba(201,168,76,0.08);border:1px solid rgba(201,168,76,0.2);border-radius:8px;padding:6px 10px;font-family:monospace;font-size:10px;color:${CHARACTER_MENU.gold}`;
  ...
  _openSlotPicker(slot, parentPanel) {
    ...
    const titleEl = document.createElement('div');
    titleEl.style.cssText = `font-family:'Cinzel',serif;font-size:13px;color:${CHARACTER_MENU.gold};letter-spacing:2px;margin-bottom:16px`;
    ...
    [null, ...items].forEach(item => {
      ...
      row.innerHTML = `
        ...
        <div style="flex:1">
          <div style="font-family:monospace;font-size:10px;color:${item ? CHARACTER_MENU.gold : 'rgba(255,100,100,0.8)'}">
    ...
    const cancelBtn = document.createElement('button');
    cancelBtn.style.cssText = `margin:16px;padding:10px 28px;border-radius:20px;border:1px solid rgba(201,168,76,0.3);background:rgba(10,8,20,0.9);color:${CHARACTER_MENU.gold};font-family:monospace;font-size:11px;cursor:pointer;pointer-events:all`;
      ...
      row.innerHTML = `
        ...
        <div style="flex:1">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
            <span style="font-family:monospace;font-size:11px;color:${CHARACTER_MENU.gold}">${skill.name ?? skill.id}</span>
            <span style="font-family:monospace;font-size:8px;padding:2px 6px;border-radius:4px;background:rgba(201,168,76,0.12);color:${CHARACTER_MENU.gold}">${(skill.rarity ?? 'common').toUpperCase()}</span>
          </div>
          ...
          <div style="font-family:monospace;font-size:8px;color:${skill.available ? CHARACTER_MENU.success : 'rgba(255,100,100,0.7)'};margin-top:3px">
  ...
  _fillTalentos(body) {
    ...
    const nameEl = document.createElement('div');
    nameEl.style.cssText = `font-family:'Cinzel',serif;font-size:13px;color:${CHARACTER_MENU.gold};letter-spacing:2px;margin-bottom:14px`;
    ...
    for (const skill of char.skills) {
      ...
      row.innerHTML = `
        <div style="font-size:28px;min-width:36px;text-align:center">${skill.icon}</div>
        <div>
          <div style="font-family:monospace;font-size:11px;color:${CHARACTER_MENU.gold};margin-bottom:4px">${skill.name}</div>
