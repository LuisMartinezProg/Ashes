...
export async function initScene() {
  ...
  // ── GRUPO MUNDO — todo el exterior aquí ──
  ...
  buildForestParticles(scene); // partículas directo a scene — se animan desde loop

  // ── ENTRADAS DE MAZMORRAS ─────────────────
  // Las entradas van en worldGroup — se ocultan al entrar
  buildDungeonEntrance(worldGroup, scene, {
    x: 0,   z: -120,
...
// ── ENTRADA DE MAZMORRA ──────────────────────────────────────────────────────
function buildDungeonEntrance(parent, scene, cfg) {
  ...
  // Luz interna — va directo a scene para que ilumine correctamente
  const caveLight = new THREE.PointLight(glow, 1.8, 10);
  caveLight.position.set(x, 2.5, z - 6);
  scene.add(caveLight);

  // Pulso de luz y niebla — animación ligera
  const startPhase = Math.random() * Math.PI * 2;
  const _pulse = () => {
    if (!parent.visible) return; // parar si mundo oculto
    const t = Date.now() * 0.001 + startPhase;
    caveLight.intensity       = 1.4 + Math.sin(t * 1.5) * 0.6;
  ...
  for (const side of [-1, 1]) {
    ...
    const torchLight = new THREE.PointLight(torchColor, 1.5, 7);
    torchLight.position.set(x + side * 3.2, 5.4, z - 4.8);
    scene.add(torchLight); // luces directo a scene
  }
  ...
  // Partículas de niebla de cueva
  const fogGeo = new THREE.BufferGeometry();
  const fogPos = new Float32Array(60 * 3);
  for (let i = 0; i < 60; i++) {
