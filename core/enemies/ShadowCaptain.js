// core/enemies/ShadowCaptain.js — Capitán de las Sombras
...
export class ShadowCaptain extends BaseEnemy {
  ...
  _buildMesh(pos) {
    ...
    this._materials.push(armorMat, darkMat);
    ...
    this._capeMat = new THREE.MeshStandardMaterial({ color: 0x1a0033, side: THREE.DoubleSide });
    const cape = new THREE.Mesh(new THREE.PlaneGeometry(0.9, 1.4), this._capeMat);
    ...
    this._visorMat = new THREE.MeshStandardMaterial({ color: 0x8800ff, emissive: 0x8800ff, emissiveIntensity: 1 });
    const visor = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.1, 0.08), this._visorMat);
  ...
  _restoreColors() {
    if (this._materials[0]) this._materials[0].color.setHex(0x080818);
    if (this._materials[1]) this._materials[1].color.setHex(0x110022);
  }
  ...
  _checkPhase2() {
    if (this._phase === 'normal' && this.hp <= this.maxHp * 0.5) {
      ...
      if (this._capeMat) this._capeMat.color.setHex(0x330011);
    }
  }
