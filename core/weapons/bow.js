...
export class BowWeapon {
  ...
  execute(hitIndex, enemies = []) {
    ...
    if (target && this._scene) this._spawnArrow(hitIndex, target);
    return !!target;
  }
  ...
  _spawnArrow(hitIndex, target) {
    const dir = target.mesh.position.clone().sub(this.player.position).setY(0).normalize();
    ...
    this._arrows.push({ mesh, direction: dir, speed: ARROW_SPEED[hitIndex], damage: DAMAGE[hitIndex], traveled: 0 });
  }

  _destroyArrow(index) {
    const a = this._arrows[index];
    if (a && this._scene) { this._scene.remove(a.mesh); a.mesh.geometry.dispose(); a.mesh.material.dispose(); }
  ...
  _playDrawAnim(hitIndex) {
    ...
    setTimeout(() => { this._bowMesh.material.color.setHex(0x886633); }, ANIM_DUR[hitIndex] * 0.4);
  }
}
